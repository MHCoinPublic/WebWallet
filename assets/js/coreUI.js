const init = () => {
    setTimeout(() => {
        ...
        console.log.bind(console, "%cDo not paste anything on this console unless you know what you are doing, otherwise, without knowing what you are doing, your money or even personal data can be stolen!", "font-size: large");
    });
    ...
    if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        document.getElementsByClassName("footer")[0].innerHTML = `MHCOin WebWallet made with
          <i class="fas fa-coffee"></i>, <i class="fas fa-code"></i> and <i class="fas fa-heart"></i> by Lapki 2021<br/>
          Background photo from Lapki`;
    }
  ...
  ws.onclose = (code) => {
    ...
    m = n.getMonth() + 1;
    d = n.getDate();
    document.getElementById("date").innerHTML = "<img height='16em' width='16em' src='https://github.com/revoxhere/duino-coin/blob/master/Resources/NewWallet.ico?raw=true'>&nbsp;MHC WebWallet (v2.2) " + d + "/" + m + "/" + y;

    if (d == 14 && m == 2) {
        document.getElementById("date").innerHTML = "❤️&nbsp;MHC WebWallet (v2.2) " + d + "/" + m + "/" + y;
    ...
    if (d == 31 && m == 10) {
        document.getElementById("date").innerHTML = "🎃&nbsp;MHC WebWallet (v2.2) " + d + "/" + m + "/" + y;
    ...
    getJSON("https://api.coindesk.com/v1/bpi/currentprice.json").then((data) => {
        ducoprice = `${data["bpi"]["USD"]["rate_float"] * 0.00000001}`;
    ...
    login.onclick = (event) => {
                ...
                .then((data) => {
                    ...
                    window.setInterval(() => {
                        fetch("https://mhc.lapki.dev/api/users/me", {
                            ...
                            .then((data) => {
                                loginstatus.innerHTML = "Успешный вход!";
                                ...
                                balanceusd = ducoprice * data["user"]["balance"];
                                balanceusd = balanceusd.toFixed(2);

                                balance = data["user"]["balance"] * 1;
                                cutbalance = balance.toFixed(6);

                                if (Number.isNaN(balanceusd)) balanceusd = " Ошибка";
                                document.getElementById("balancetext").innerHTML = cutbalance + " Պ ($" + balanceusd + ")";
        ...
        //     document.getElementById("jwt").text = jwtssf2211
        //     window.setInterval(() => {
    ...
    send.onclick = (event) => {
        ...
        if (amount && recipient) {
            fetch("https://mhc.lapki.dev/api/users/transfer", {
                headers: {
                    Authorization: document.getElementById("jwt").text,
                    "Content-Type": "application/json;charset=utf-8",
                },
                ...
                .then((data) => {
                    if (data["status"] == "ok") {
                        alert("Успешно отправил средства!");
                    } else {
